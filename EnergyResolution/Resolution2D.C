...
void Resolution2D(TString PeakToFit, Double_t MaxRes)
{
...
/////////////////////////////////////////////////////////////////////////////
    ...
    for(Int_t i=0; i<32; i++) {
      ...
      if(FResolution<0) {
        if(FResolution<-1.5) {  // Strip is broken
          ...
          ResHist->Fill(FStripNo, det+1.25, Int_t(1000*MaxRes));
        }
      } // end "flag" if
      ...
      if(FResolution>0) { // Value to read
        ResHist->Fill(FStripNo, det+1.25, Int_t(1000*FResolution));
      ...
      if(BResolution>0) { // Value to read
        ResHist->Fill(BStripNo, det+0.75, Int_t(1000*BResolution));
  ...
  // Save in pdf format
  TString Canvas_SaveNamePDF = "Resolution2D";
  ...
  // Save in png format
  TString Canvas_SaveNamePNG = "Resolution2D";
